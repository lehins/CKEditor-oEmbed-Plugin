...
(function () {
	...
	init: function (editor) {
	    // Load jquery?
	    loadjQueryLibaries(editor.config.oembed_providers);
	    ...
	    function loadjQueryLibaries(providers) {
		function loadjQueryOEmbed(){
		    if (typeof(jQuery.fn.oembed) === 'undefined') {
			CKEDITOR.scriptLoader.load(CKEDITOR.getUrl(
			    CKEDITOR.plugins.getPath('oembed') +  'libs/jquery.oembed.min.js'
			), function(success){
			    if(success && typeof(providers) !== 'undefined'){
				var filtered = [];
				for (var i=0; i < jQuery.fn.oembed.providers.length; i++){
				    if (providers.indexOf(jQuery.fn.oembed.providers[i].name) >=0){
					filtered.push(jQuery.fn.oembed.providers[i]);
				    }
				}
				jQuery.fn.oembed.providers = filtered;
			    }
			});
		    }
		}
		...
		return {
		    ...
		    contents: [{
			label: "Embed Media Content (Video, Audio)",
			id: 'general',
			elements: [{
				type: 'html',
				id: 'oembedHeader',
				html: '<div style="white-space:normal;width:500px;padding-bottom:10px">Paste a URL from one of the supported sites. <br>We currently support embedding media from these websites: <a href="http://www.youtube.com" target="_blank">Youtube</a>, <a href="http://www.vimeo.com" target="_blank">Vimeo</a>, <a href="http://www.soundcloud.com" target="_blank">Soundcloud</a></div>'
			    }, {
				type: 'text',
				id: 'embedCode',
				focus: function() {
